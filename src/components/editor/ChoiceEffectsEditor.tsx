...
import type { Effect, EffectAction, StoryVariables } from "../../types/storyCore";

const ACTIONS: { value: EffectAction; label: string }[] = [
  { value: "set", label: "set" },
  { value: "add", label: "add" },
...
export interface ChoiceEffectsSource {
  effects?: Effect[] | null;
}

export interface ChoiceEffectsEditorProps {
  choice: ChoiceEffectsSource;
  variables?: StoryVariables;
  onUpdate: (field: "effects" | string, value: Effect[]) => void;
}
...
export default function ChoiceEffectsEditor({
  ...
  useEffect(() => {
    ...
    if (currentCount > previousCount && currentCount > 0) {
      ...
      expandedEffectIndex !== null &&
      expandedEffectIndex >= currentCount
    ) {
  ...
  function renderValueInput(effect: Effect, index: number) {
